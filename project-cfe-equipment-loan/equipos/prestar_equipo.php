...
// Obtener información del equipo (y verificar que el usuario actual es el responsable)
$rpe_responsable = $_SESSION["RPE"]; // El usuario actual es el responsable
...
$stmt_equipo->bind_param('ss', $numero_serie, $rpe_responsable);
$stmt_equipo->execute();
$result_equipo = $stmt_equipo->get_result();

if ($result_equipo->num_rows === 0) {
    header("Location: gestion_equipos.php?error=No tienes permisos para prestar este equipo o no está disponible");
    exit();
}

$equipo = $result_equipo->fetch_assoc();

// El usuario actual es el responsable
$nombre_responsable = $_SESSION["nombre"] ?? '';
$departamento_equipo = $equipo['Departamento'];
$division_equipo = $equipo['Division'];

// Obtener lista de usuarios para seleccionar solicitante
$query_usuarios = "SELECT RPE, nombre, departamento, division FROM usuarios WHERE RPE != ? ORDER BY nombre";
$stmt_usuarios = $conn->prepare($query_usuarios);
$stmt_usuarios->bind_param('s', $rpe_responsable);
...
$tiempo_actual = time();
$tiempo_por_defecto = $tiempo_actual + (5 * 60); // 5 minutos en el futuro
...
<head>
    ...
    <title>Prestar Equipo - Sistema de Inventarios</title>
    <link rel="stylesheet" href="prestar_equipo.css">
</head>
<body>
    <div class="main-container">
        ...
        <div class="content">
            <div class="page-header">
                <h2>Prestar Equipo a Otro Usuario</h2>
                <p>Como responsable, puedes prestar este equipo a otro usuario</p>
            </div>
            ...
            <div class="info-section">
                ...
                <div class="info-grid">
                    ...
                    <div class="info-item">
                        <div class="info-label">Responsable (Tú)</div>
                        <div class="info-value">
            ...
            <!-- Formulario para seleccionar solicitante, fechas y observaciones -->
            <form action="../includes/procesar_prestamo_directo.php" method="POST" id="prestamoForm">
                ...
                <div class="form-section">
                    ...
                    <div class="form-group">
                        <label for="rpe_solicitante" class="required">Seleccionar Solicitante</label>
                        <div class="user-search">
                            <input type="text" 
                                   id="buscar_usuario" 
                                   placeholder="Buscar usuario por nombre o RPE..."
                                   onkeyup="filtrarUsuarios()">
                            <div class="user-search-icon">🔍</div>
                        </div>
                        <select id="rpe_solicitante" 
                                name="rpe_solicitante" 
                                class="filter-select" 
                                required>
                            <option value="">-- Seleccione un usuario --</option>
                            <?php while ($usuario = $result_usuarios->fetch_assoc()): ?>
                                <option value="<?php echo htmlspecialchars($usuario['RPE']); ?>"
                                        data-nombre="<?php echo htmlspecialchars($usuario['nombre']); ?>"
                                        data-departamento="<?php echo htmlspecialchars($usuario['departamento'] ?? ''); ?>"
                                        data-division="<?php echo htmlspecialchars($usuario['division'] ?? ''); ?>">
                                    <?php echo htmlspecialchars($usuario['nombre']); ?> 
                                    (<?php echo htmlspecialchars($usuario['RPE']); ?>) - 
                                    <?php echo htmlspecialchars($usuario['departamento'] ?? 'Sin departamento'); ?>
                                </option>
                            <?php endwhile; ?>
                        </select>
                        <div id="info_usuario" style="margin-top: 15px; display: none;">
                            <div class="info-item">
                                <div class="info-label">Información del solicitante seleccionado:</div>
                                <div class="info-value">
                                    <span id="nombre_usuario"></span> 
                                    (<span id="rpe_usuario"></span>)<br>
                                    <small>Depto: <span id="depto_usuario"></span> | 
                                    División: <span id="div_usuario"></span></small>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="buttons">
                    <button type="button" class="btn btn-cancel" onclick="cancelarPrestamo()">
                        Cancelar
                    </button>
    ...
    <script>
        ...
        document.getElementById('rpe_solicitante').addEventListener('change', function() {
            const selectedOption = this.options[this.selectedIndex];
            const infoDiv = document.getElementById('info_usuario');
            
            if (this.value) {
                document.getElementById('nombre_usuario').textContent = selectedOption.getAttribute('data-nombre');
                document.getElementById('rpe_usuario').textContent = this.value;
                document.getElementById('depto_usuario').textContent = selectedOption.getAttribute('data-departamento') || 'No asignado';
                document.getElementById('div_usuario').textContent = selectedOption.getAttribute('data-division') || 'No asignado';
        ...
        function filtrarUsuarios() {
            ...
            const select = document.getElementById('rpe_solicitante');
        ...
        document.getElementById('prestamoForm').addEventListener('submit', function(e) {
            const fechaInicioInput = document.getElementById('fecha_inicio');
            const fechaFinInput = document.getElementById('fecha_fin');
            const solicitante = document.getElementById('rpe_solicitante').value;
            ...
            if (!solicitante) {
                e.preventDefault();
                alert('Error: Debe seleccionar un solicitante.');
                return false;
            }
        ...
        function cancelarPrestamo() {
            if (confirm('¿Está seguro de que desea cancelar el préstamo?')) {
